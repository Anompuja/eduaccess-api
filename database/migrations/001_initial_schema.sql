...
-- Source  : Migrated & redesigned from KitaGiat (MariaDB)
--
-- Changes from KitaGiat:
--   • All INT/BIGINT AUTO_INCREMENT PKs → UUID gen_random_uuid()
--   • All CHAR(36) UUID columns → native UUID type
--   • TINYINT(1) → BOOLEAN
--   • ENUM → VARCHAR(50) CHECK (col IN (...))
--   • LONGTEXT / TEXT JSON columns → JSONB
--   • DATETIME / TIMESTAMP → TIMESTAMPTZ
--   • Removed ENGINE=InnoDB, CHARSET, COLLATE directives
--   • Backtick identifiers → double-quote identifiers
--   • school_id added to all business tables (tenant scoping)
--   • headmaster_profiles kept separate (confirmed by user)
--   • student_profiles.parent_id replaced by student_parent_links
--   • subscriptions: polymorphic billable_* → direct school_id FK
--   • cbt / cbt_campaigns: school_id added
--   • refresh_tokens table added (JWT)
--   • System/Laravel-internal tables skipped entirely
--   • Indonesian ENUM values translated to English
--   • 'uknown' typo corrected to 'unknown' in classrooms.status
-- ============================================================
